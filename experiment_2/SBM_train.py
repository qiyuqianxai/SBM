...
def node_train(node_id, node_model, data, test_data, args):
    ...
    node_train_dataloader = DataLoader(node_train_datasets, batch_size=args.batch_size, shuffle=True,
    ...
    for node_epoch in range(args.node_epochs):
        ...
        scheduler.step()
        print(
            f"node{node_id} epoch{node_epoch}:node_model train_loss:{sum(epoch_loss) / len(epoch_loss)},train_acc:{sum(epoch_acc) / len(epoch_acc)}")
...
def SBM(model, all_node_data, test_data, args):
    ...
    for iteration in range(args.iterations):
        ...
        if args.select_method == "rl":
            ...
            if iteration > 0 and np.random.uniform() > acc_record[-1] * a_ratio / cost_energy_record[
                -1]:  # choosing action
            ...
            # 使用差分进化算法
            try:
                select_nodes = NodeChoose(cp_matrix, tp_matrix, ef_matrix, args.node_num, args.select_node_num,
                                          args.b_ratio)
        ...
        for i in range(args.node_num):
            ...
            info, tmp_loss_record, tmp_acc_record = node_train(i, copy.deepcopy(model).to(args.device),
                                                               all_node_data[i], test_data, args)
        ...
        if args.select_method == None:
            ...
            for k in w_avg.keys():
                ...
                for i in range(1, len(per_iter_nodes_res)):
                    w_avg[k] += per_iter_nodes_res[i]["node_model_data"][k] * len(
                        all_node_data[per_iter_nodes_res[i]["node_id"]])
        ...
        per_iter_test_loss_record, per_iter_test_acc_record = SBM_test(args, copy.deepcopy(model).to(args.device),
                                                                       test_data)
